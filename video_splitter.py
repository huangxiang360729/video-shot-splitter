"""Unified local video shot splitting pipeline.

The project intentionally keeps only two Python entrypoints:
- video_splitter.py: all local video analysis, candidate fusion, and report generation
- app.py: the upload/progress web UI
...
_EMBEDDED_MODULES: dict[str, str] = {}


def _load_embedded_module(name: str) -> types.ModuleType:
    if name in sys.modules:
        return sys.modules[name]
    source = _EMBEDDED_MODULES[name]
...
def _run_stage(module_name: str, argv: list[str]) -> None:
    original_argv = sys.argv[:]
    try:
        sys.argv = [f"video_splitter.py {module_name}", *argv]
        _load_embedded_module(module_name).main()
    finally:
        sys.argv = original_argv


def main() -> None:
    parser = argparse.ArgumentParser(description="Unified local video shot splitting pipeline.")
    ...
    if args.stage == "transnet":
        _run_stage("split_shots_transnetv2", args.stage_args)
    elif args.stage == "autoshot":
        _load_embedded_module("visualize_frame_transitions")
        _run_stage("run_shotsplit_autoshot", args.stage_args)
    elif args.stage == "sweep":
        _run_stage("candidate_sweep_local", args.stage_args)



# ---- embedded stage: visualize_frame_transitions.py ----
_EMBEDDED_MODULES['visualize_frame_transitions'] = r'''
import argparse
import csv
import html
import json
...
# ---- embedded stage: split_shots_transnetv2.py ----
_EMBEDDED_MODULES['split_shots_transnetv2'] = r'''
import argparse
import csv
import html
import json
...
# ---- embedded stage: candidate_sweep_local.py ----
_EMBEDDED_MODULES['candidate_sweep_local'] = r'''
import argparse
import csv
import html
import json
...
# ---- embedded stage: run_shotsplit_autoshot.py ----
_EMBEDDED_MODULES['run_shotsplit_autoshot'] = r'''
import argparse
import csv
import json
from pathlib import Path
